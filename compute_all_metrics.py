...
# All conditions with their metadata: (condition_label, epsilon, method)
CONDITIONS = {
    "clean_no_defense":          ("A", "none", "none"),
    "adv_eps4_no_defense":       ("B", "4.0",  "none"),
    "adv_eps8_no_defense":       ("B", "8.0",  "none"),
    "adv_eps4_defense_gaussian": ("C", "4.0",  "gaussian"),
    "adv_eps4_defense_median":   ("C", "4.0",  "median"),
    "adv_eps4_defense_bilateral":("C", "4.0",  "bilateral"),
    "adv_eps8_defense_gaussian": ("C", "8.0",  "gaussian"),
    "adv_eps8_defense_median":   ("C", "8.0",  "median"),
    "adv_eps8_defense_bilateral":("C", "8.0",  "bilateral"),
}
...
def read_benchmark_log(exp_run_dir):
    """
    Parse PSNR, SSIM, Gaussian count and training time from
    benchmark_result.log written by benchmark.py.
    """
    ...
    with open(log_path, "r") as f:
        for line in f:
            ...
            if m:
                result["n_gaussians"] = int(float(m.group(1)) * 1_000_000)

            m = re.search(r"Training time\s*:\s*([\d.]+)", line, re.IGNORECASE)
            if m:
                result["training_time_min"] = float(m.group(1))
...
def clip_scores(img_pil, true_label, device):
    ...
    top1_hit  = int(top3_idx[0] == true_idx)
    top3_hit  = int(true_idx in top3_idx)
    return true_conf, top1_hit, top3_hit
    ...
    for cond, (cond_label, eps, method) in CONDITIONS.items():
        ...
        for obj in NERF_CLASSES:
            ...
            if clip_conf is None:
                print(f"[WARN: no renders found] ", end="")
            ...
            rows.append({
                ...
                "object":             obj,
                "psnr":               bench["psnr"]              if bench["psnr"]              else "",
                "ssim":               bench["ssim"]              if bench["ssim"]              else "",
                ...
                "n_gaussians":        bench["n_gaussians"]       if bench["n_gaussians"]       else "",
                "training_time_min":  bench["training_time_min"] if bench["training_time_min"] else "",
            })
