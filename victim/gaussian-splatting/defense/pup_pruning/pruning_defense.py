"""
Smart Pruning Defense Against Poison-Splat Computation Cost Attacks.

This module provides a gradient-based importance pruning mechanism inspired by
PUP 3D-GS (Hanson et al., CVPR 2025) that can be integrated into the victim
3D-GS training loop within the poison-splat repo.

Unlike the naive defense (which just caps the max number of Gaussians and
causes severe quality degradation), this defense uses accumulated gradient
magnitudes of the spatial (xyz) and scaling parameters as a lightweight proxy
for PUP 3D-GS's Fisher-based sensitivity score. This allows it to
intelligently prune the LEAST important Gaussians while preserving the ones
that matter most for reconstruction quality.

Placement in poison-splat repo:
    victim/gaussian-splatting/defense/smart_pruning/pruning_defense.py

Reference:
    - PUP 3D-GS: Hanson et al., CVPR 2025
    - Poison-Splat: Lu et al., ICLR 2025
"""
...
class SmartPruningDefense:
    """
    Gradient-based importance pruning defense for 3D Gaussian Splatting.

    Instead of naively capping the Gaussian count (which destroys quality),
    this defense periodically scores every Gaussian by how much the
    reconstruction loss is sensitive to its position and scale, then prunes
    the bottom-K% lowest-scoring ones.

    Args:
        max_gaussians:           Hard upper bound. Triggers immediate pruning.
        soft_budget:             Target count after budget-triggered pruning.
        pruning_interval:        Check every N iterations.
        pruning_start_iter:      Don't prune before this (warmup).
        growth_rate_threshold:   Flag attack if count grows by this factor.
        aggressive_prune_ratio:  Fraction to prune when attack is detected.
        normal_prune_ratio:      Fraction to prune in periodic maintenance.
        score_type:              'gradient' | 'opacity_volume' | 'hybrid'
        verbose:                 Print defense log messages.
    """
