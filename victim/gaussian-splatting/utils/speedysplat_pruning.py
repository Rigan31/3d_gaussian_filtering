#
# SpeedySplat-style Pruning Defense for Poison-Splat
#
# Implements the Efficient Pruning Score from:
#   "Speedy-Splat: Fast 3D Gaussian Splatting with Sparse Pixels and Sparse Primitives"
#   Alex Hanson, Allen Tu, Geng Lin, Vasu Singla, Matthias Zwicker, Tom Goldstein
#   CVPR 2025
#
# The pruning approach has two phases:
#   1. Soft Pruning: During densification, at each densification step, prune
#      the bottom X% of Gaussians by the Efficient Pruning Score. This runs
#      alongside the normal clone/split/prune cycle of 3DGS.
#   2. Hard Pruning: After densification ends, periodically prune an additional
#      Y% of Gaussians using the same score.
#
# The Efficient Pruning Score is derived from PUP 3D-GS's sensitivity score
# but made memory-efficient (36x reduction). It uses only the 6 spatial
# parameters (3 mean + 3 scale) to compute a per-Gaussian sensitivity:
#
#   score_i = log det( diag(grad_mean_i^2 + grad_scale_i^2) )
#
# This captures geometric sensitivity: how much the reconstruction loss
# changes when a Gaussian's position or scale is perturbed. Gaussians with
# low sensitivity contribute little to reconstruction and can be safely pruned.
#
# For Poison-Splat defense, this is particularly effective because:
#   - Poison-induced Gaussians fit high-frequency noise textures
#   - They tend to have chaotic gradients that average out to low sensitivity
#   - The soft pruning during densification prevents the Gaussian count explosion
#     that is the core mechanism of the computation cost attack
#
...
class SpeedySplatPruningDefense:
    """
    Defense mechanism integrating SpeedySplat's pruning into the
    Poison-Splat training loop.

    Two-phase pruning:
      Phase 1 - Soft Pruning (during densification):
        At every densification step, after the normal densify_and_prune,
        compute the Efficient Pruning Score and prune the bottom
        soft_prune_percent of Gaussians.

      Phase 2 - Hard Pruning (after densification ends):
        At specified iterations, compute the score across all training
        views and prune the bottom hard_prune_percent of Gaussians.

    Args:
        soft_prune_percent: Fraction to prune at each densification step (default 0.1 = 10%)
                           SpeedySplat uses ~80% total during densification across many steps,
                           but we use smaller per-step amounts since we prune every step.
        hard_prune_percent: Fraction to prune at each hard prune iteration (default 0.3 = 30%)
        hard_prune_iterations: List of iterations for hard pruning (after densification)
        score_accumulation_views: Number of random views to sample for score computation
                                 during soft pruning (for efficiency; hard pruning uses all views)
        max_gaussians: Hard safety cap
    """
    ...
    def accumulate_gradients(self, gaussians):
        """
        Accumulate squared gradients of mean (xyz) and scale parameters.

        This should be called every iteration AFTER loss.backward() and
        BEFORE optimizer.step() / zero_grad().

        The Efficient Pruning Score from SpeedySplat approximates the
        Fisher Information / Hessian diagonal using only the 6 spatial
        parameters (3 mean + 3 scale), computing:
            H_i ≈ diag( E[ (∂L/∂μ_i)^2, (∂L/∂s_i)^2 ] )
        """
    ...
    def compute_efficient_pruning_score(self, gaussians):
        """
        Compute the SpeedySplat Efficient Pruning Score.

        score_i = log det( diag( avg_grad_mean_i^2, avg_grad_scale_i^2 ) )
                = sum( log( avg_grad_mean_i^2 + eps ) ) + sum( log( avg_grad_scale_i^2 + eps ) )

        This is equivalent to taking the log determinant of the diagonal
        Fisher information matrix restricted to spatial parameters.

        Higher score = more sensitive to perturbation = more important for reconstruction.
        Lower score = less important = candidate for pruning.

        Returns:
            scores: [N] tensor, per-Gaussian sensitivity score
        """
        eps = 1e-12
    ...
    def soft_prune(self, gaussians, iteration):
        """
        Soft Pruning: Called at each densification step during training.

        Computes the Efficient Pruning Score from accumulated gradients
        and removes the bottom soft_prune_percent of Gaussians.

        This is the key defense mechanism against Poison-Splat: by pruning
        low-sensitivity Gaussians at every densification step, we prevent
        the explosive Gaussian growth that the attack relies on.
